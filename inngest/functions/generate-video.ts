  ...
  async ({ event, step }) => {
    ...
    if (video?.videoUrl && plunk) {
      await step.run("send-email", async () => {
        ...
        if (userErr || !user?.email) return { sent: false };
        ...
        const result = await plunk!.emails.send({
          to: user.email,
          from: process.env.PLUNK_FROM_EMAIL || "hello@vediomax.com",
    ...
    if (project.platform === "youtube" && video?.videoUrl) {
      await step.run("publish-youtube", async () => {
        console.log(`Publishing project ${projectId} to YouTube...`);
        ...
        if (intError || !integration) {
          throw new Error("YouTube integration not found for this user");
        }
        ...
        // 3. Refresh Token if needed (explicitly check or let library handle, but library needs refresh_token)
        // The library handles refresh if refresh_token is present and access_token expired.
        // We should listen to 'tokens' event to save new tokens, but in serverless/Inngest it's linear.
        // We can force a check or just try. Best to refresh manually if close to expiry to save to DB.

        const now = Date.now();
        const expiry = new Date(integration.expires_at).getTime();
        if (expiry <= now + 60000) { // 1 minute buffer
          console.log("Refreshing YouTube access token...");
          const { credentials } = await oauth2Client.refreshAccessToken();

          // Update DB
          await supabaseAdmin.from("social_integrations").update({
            access_token: credentials.access_token,
            expires_at: new Date(credentials.expiry_date!).toISOString(),
            // refresh_token might not be returned if not rotated
          }).eq("id", integration.id);

          oauth2Client.setCredentials(credentials);
        }

        // 4. Download Video (YouTube API needs a stream or buffer)
        ...
        const res = await youtube.videos.insert({
          ...
          requestBody: {
            snippet: {
              title: scriptData.videoTitle.substring(0, 100), // Max 100 chars
              description: scriptData.script.substring(0, 5000) + "\n\nGenerated by VideoMax",
              tags: ['VideoMax', 'AI', 'Generated'],
        ...
        return { published: true, videoId: res.data.id };
      });
    }

    // Step 9: Publish to Instagram (Placeholder)
    if (project.platform === "instagram" && video?.videoUrl) {
      await step.run("publish-instagram", async () => {
        console.log(`[Placeholder] Publishing project ${projectId} to Instagram...`);
        // TODO: Implement Instagram Graph API upload
        return { published: false, reason: "Not implemented" };
      });
    }

    // Step 10: Publish to TikTok (Placeholder)
    if (project.platform === "tiktok" && video?.videoUrl) {
      await step.run("publish-tiktok", async () => {
        console.log(`[Placeholder] Publishing project ${projectId} to TikTok...`);
